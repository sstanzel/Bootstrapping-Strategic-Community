---
title: "Diplaucus Full Script"
format: html
editor: visual
---

# Bootstrapping Megascript ヽ(•‿•)ノ
...
species_list <- c("Diplaucus aurentiacus") #CHANGE
...
```{r}
results_dir <- "~/R/Results/Diplaucus/09Jul25" # <-- CHANGE this to your desired output directory
data_dir <- "~/R/Data/Diplaucus/09Jul25" # <-- CHANGE, this is where you will put all the data you used for this SDM
...
### Species Occurrence Data ღƪ(ˆ◡ˆ)ʃ♡ƪ(ˆ◡ˆ)ʃ♪
...
```{r}
data_raw <- read_tsv("~/R/Data/Diplacus_aurantiacus.csv") #CHANGE to whatever your file path is. You can also change
...
TBH you are probably going to skip the next \~1000 lines, you should just go to the Bootstrapping section, everything will work straight from there

## Check Co-linearity of Variables ¯\\\_(ツ)\_/¯
...
### Replicate Function to nest inside full bootstrap -===≡≡≡┌( ͝° ͜ʖ͡°)┘

there are two versions of this function, one that only saves the models and one that only saves the predictions and the VI values. there are also two bootstrapping functions because each calls a different version of the replicate function. prob you can just use the second version of both

```{r}
replicate_run <- function(x, n_bootstrap, species_name, predictors, modelmethod, clean_occ_sf, bg_df) { #do this for however many times you want to bootstrap
  ...
  predictors <- unwrap(predictors)
    ...
    # i = i+1
  message("replicate_run() for ", n_bootstrap, "iterations")
    ...
    occ_boot <- clean_occ_sf |> #new sf 
      ...
      group_by(cell) |> #chosen randomly SHOULD THIS BE HERE??
  ...
  point_count <- nrow(as.data.frame(sdm_data_pts))
    ...
    # varimp_rep <- varImp(model, permut = 5) #stores variable importance for each model
  
    # prediction_rep <- predict(model, data = predictors, type = "cloglog") #creates a prediction for each model and stores it
    
    # message("---- Done with iteration ", i, "-----")
    
    # final_model <- model #some of the functions only work on models, not predictions, so storing the last model makes it easy to call it later. I had to change this once the loop was deleted - now it stores every model as the final one but only temporarily, so onyl the last one remains as the final_model
    # output_list <- list(varimp = varimp_rep, prediction = prediction_rep, model = model)
    # output_list <- list(model = model)
    return(model)
}
```

Version 2, which does vimp and predictions in addition to the models:

```{r}
replicate_run_2 <- function(x, n_bootstrap, species_name, predictors, modelmethod, clean_occ_sf, bg_df, bootstrap_dir) { #do this for however many times you want to bootstrap
  ...
  pred_path <- file.path(bootstrap_dir, paste0("prediction_", x, ".tif"))
  writeRaster(prediction_rep, pred_path, overwrite = TRUE)
  
  
  # message("---- Done with iteration ", i, "-----")
  
  # final_model <- model #some of the functions only work on models, not predictions, so storing the last model makes it easy to call it later. I had to change this once the loop was deleted - now it stores every model as the final one but only temporarily, so onyl the last one remains as the final_model
  output_list <- list(varimp = varimp_rep, prediction_path = pred_path, model = model)
  
  return(output_list)
}
```

### Bootstrap Function (▀̿Ĺ̯▀̿ ̿)
...
bootstrap_sdm <- function(
  ...
  model_list <- 1:n_bootstrap|>
    future_map(.f = replicate_run, n_bootstrap=n_bootstrap, species_name=species_name, predictors=predictors, modelmethod=modelmethod,clean_occ_sf=clean_occ_sf, bg_df=bg_df, .progress = TRUE, .options = furrr_options(seed = TRUE))
...
Version 2, which takes replicate_run_2 instead:

```{r}
bootstrap_sdm_2 <- function(
    species_name, clean_occ_sf, predictors, results_dir,
    n_bg_points = 10000, modelmethod = "Maxnet",
    n_bootstrap = 10) {
  message("\n--- Modeling: ", species_name, " ---")  
  ...
  model_list <- 1:n_bootstrap|>
    future_map(.f = replicate_run_2, bootstrap_dir = bootstrap_dir, n_bootstrap=n_bootstrap, species_name=species_name, predictors=predictors, modelmethod=modelmethod,clean_occ_sf=clean_occ_sf, bg_df=bg_df, .progress = TRUE, .options = furrr_options(seed = TRUE))
...
```{r}
diplaucus_model_list <- bootstrap_sdm_2(
...
#### Pull info ᕙ( ͡° ͜ʖ ͡°)ᕗ

it errored out for some reason but it did save all of the predictions so i am just calling them manually

```{r}
#predictions_list <- vector(mode='list', length=1000)

# for (x in 1:1000){
#   pred_path <- file.path('~/R/Results/Diplaucus/09Jul25/bootstrapped_Diplaucus_aurentiacus', paste0("prediction_", x, ".tif"))
#   predictions_list[[x]] <- pred_path
# }

predictions_path_list <- lapply(diplaucus_model_list, function(x) x$prediction_path) #dont need to do this, just to clarify the process

# files <-  list.files('~/R/Results/Diplaucus/09Jul25/bootstrapped_Diplaucus_aurentiacus', pattern = ".", all.files = FALSE, recursive = TRUE, full.names = TRUE)

length(predictions_path_list) #BRO WHATTTTTTTTT WHY ARE THERE ONLY 787 WHY IS 14-50 MISSING WHATTTT

raster_stack <- rast(lapply(predictions_list, function(x) rast(x)))



varimp_df <- do.call(rbind, lapply(seq_along(diplaucus_model_list), function(i) {
  df <- diplaucus_model_list[[i]]$varimp
  df$model_id <- i
  df
}))
...
mean_pred <- mean(raster_stack)
writeRaster(mean_pred, file.path(results_dir, "mean_pred.tif"), overwrite = TRUE)

sd_pred <- stdev(raster_stack)
writeRaster(sd_pred, file.path(results_dir, "sd_pred.tif"), overwrite = TRUE)
```

##### Predictions, Mean and SD function (┛ಸ_ಸ)┛彡┻━┻

this doesnt work just ignore that it exists, im just keeping it around for sentimental purposes (ಥ ͜ʖಥ)

```{r}
bootstrap_predictions <- function(model_list, predictors, results_dir, species_list)
  {
  ...
  #   function(i) {
  #     preds <- terra::predict(unwrap(predictors), model_list[[i]], na.rm = TRUE)
  #     return(preds)
  #     }
  #   )
  
  predictions_list <- model_list|>
    future_map(\(model_list) predict(model = model_list, object=predictors, type="cloglog"))
  
  # predictions_list <- future_map(model_list, ~predict(., data = predictors, type = "cloglog"))
  
  toc()
  
  # predictions_list <- 1:n_bootstrap|>
    # future_map(.f = predict, data=predictors, type="cloglog", .progress = TRUE)
  
  plan(sequential)
  
  #save stuff
  predictions_rast <- rast(predictions_list)
  writeRaster(predictions_rast, file.path(bootstrap_dir, "bootstrap_predictions.tif"), overwrite = TRUE)

  #mean and sd plots
  mean_prediction <- terra::app(predictions_rast, mean, na.rm = TRUE)
  sd_predictions <- terra::app(predictions_rast, sd, na.rm = TRUE)
  
  mean_plot <- tm_shape(mean_prediction) +
    tm_raster(palette = "viridis", title = "Mean Prediction") +
    tm_layout(main.title = "Mean Prediction")
  
  sd_plot <- tm_shape(sd_predictions) +
    tm_raster(palette = "-Greys", title = "Prediction SD") +
    tm_layout(main.title = "Uncertainty (SD)")
  
  writeRaster(mean_prediction, file.path(bootstrap_dir, "bootstrap_mean_prediction.tif"), overwrite = TRUE)
  writeRaster(sd_predictions, file.path(bootstrap_dir, "bootstrap_sd_prediction.tif"), overwrite = TRUE)
  tmap_save(mean_plot, file.path(bootstrap_dir, "mean_prediction.png"), width = 800, height = 600)
  tmap_save(sd_plot, file.path(bootstrap_dir, "prediction_sd.png"), width = 800, height = 600)
  
  return(list(
    predictions_rast = predictions_rast,
    mean_prediction = mean_prediction,
    sd_prediction = sd_prediction
  ))
}

bootstrap_predictions(quercus_model_list, predictors, results_dir, species_list)
```

### VI

This just takes varimp_df from above and does a bunch to it and makes to pretty yayyyy
...
vi_plot <- 
  ...
  labs(
    title = paste("Bootstrap Variable Importance:", species_list),
    x = "", 
...
write.csv(varimp_summary, file.path(results_dir, "bootstrap_var_imp_summary.csv"), row.names = FALSE)

ggsave(file.path(results_dir, "bootstrap_varImp_boxplot.png"),
         plot = vi_plot, width = 8, height = 6)
  
# If you run bootstrap_sdm OG you can name one of the functions final_model and run these safe_response_curves code on it
...
pred_stack <- raster_stack
```

Now take a look at this beauty:

```{r}
bootstrap_advanced_analysis <- function(pred_stack, results_dir) {
...
### Pretty up a final graph ƪ(♥ﻬ♥)ʃ

```{r}
#you dont have to do this part, mine just decided to forget what these were for some reason
mean_prediction <- terra::rast("~/R/Results/Quercus/08Jul25/mean_pred.tif")
sd_prediction <- terra::rast("~/R/Results/Quercus/08Jul25/sd_pred.tif")

mean_df <- as.data.frame(mean_pred, xy = TRUE, na.rm = TRUE)
sd_df <- as.data.frame(sd_pred, xy = TRUE, na.rm = TRUE)
...
```{r}
combined_rast <- c(mean_pred, sd_pred)
...
Hackjob of a function that is just a fran

```{r}
threshold_prep <- function(clean_occ_sf, modelmethod, predictors, species_name){
  ...
  point_count <- nrow(as.data.frame(sdm_data_pts))
    ...
    threshold_prediction <- predict(model, data = predictors, type = "cloglog")

    
    threshold_data <- replicate_SWD
    threshold_model <- model
    
    threshold_list <- c(threshold_data, threshold_model, threshold_prediction)
    
    return(threshold_list)
}

threshold_info <- threshold_prep(clean_occ_sf, modelmethod='Maxnet', predictors = predictors, species_name = species_list)

threshold_data <- threshold_info[[1]]
threshold_model <- threshold_info[[2]]
threshold_prediction <- threshold_info[[3]]
```
...
threshold <- SDMtune::thresholds(
  model = threshold_model,
  type = "cloglog",
  test = threshold_data) #this is a return from the bootstrap function, and represents teh data used for ONLY THE FINAL model

threshold_value <- threshold$`Cloglog value`[threshold$Threshold == "Maximum test sensitivity plus specificity"]

threshold_polygon <- classify(mean_pred, 
...
One day i will make these into a funciton, but for now you can mindlessly chug through them
...
sd_polygon_plot <- ggplot() +
  ...
  geom_raster(data = sd_df, aes(x = x, y = y, fill = std)) +
...
```{r}
sd_values <- values(sd_pred)
high_variance_threshold <- quantile(sd_values, 0.75, na.rm = TRUE) #create cutoff for 75%


presence_mask <- classify(mean_pred, rbind(c(-Inf, threshold_value, 0), c(threshold_value, Inf, 1)))
variance_mask <- classify(sd_pred, rbind(c(-Inf, high_variance_threshold, 0), c(high_variance_threshold, Inf, 1)))
...
intersection_plot <- ggplot() +
  ...
  geom_sf(data = combined_poly, fill = "skyblue", alpha = 0.01, color = "blue") +
...
I have a data frame with polygons and a column called ownership level, including charcter values like City, County, Federal, etc. I want to make a new column called Class that says if each polygon is either public or private. how do i do that in R?

how do i edit the df containing geometries and class into a raster of just public and private lands, projected to crs epsg 4326

i have a 

### Load Data
```{r}
land_ownership <- st_read('~/R/Data/landownership.gpkg')|>
  mutate(Class = case_when(
    OWN_LEVEL %in% c("City", "County", "Federal", "State", "Tribal") ~ "Public",
    TRUE ~ "Private"))|>
  rename('Ownership Level' = 'OWN_LEVEL')|>
  select('Ownership Level', Class, geom)
  
land_class <- land_ownership|>
  select(Class, geom)

plot(land_class)
```

i have two sfs: one that has land ownership categories public and private, and one that has a species distribution. i want to intersect the two to find the amount of land that the species exists on that is public vs private. i only have land ownership data for california, and the species goes over the border a little on each n/s end, but that's okay i only want to know the info for california. how do i do this in r?

### Graph
```{r}
land_class <- st_transform(land_class, crs = crs_epsg)
...
intersection_plot <- ggplot() +
  ...
  # scale_fill_viridis_c(option = "magma", name = "Mean Prediction") +
  # Background Environemtnal variable
  geom_sf(data = land_class, aes(fill = Class)) +
  scale_fill_manual(values = c("Public" = "forestgreen", "Private" = "tan"),
                  name = "Land Ownership")+
...
### Intersect
```{r}
truth <- st_is_valid(land_class) 
unique(truth) #should return true

st_is_valid(combined_poly) #should return true

isTRUE(crs(land_class)==crs(combined_poly)) #should return true


sf_use_s2(FALSE) #this feels gross but for some reason it is also the only thing that works so there you have it

intersection_ownership <- st_intersection(land_class, combined_poly)

sf_use_s2(TRUE) #DO THIS



intersection_overlap_dist <- st_transform(intersection_ownership, 3310) #into a distance format

# Calculate area
intersection_overlap_dist$area_sqkm <- as.numeric(st_area(intersection_overlap_dist))#in m², divide by 1e6 to km²

summary <- intersection_overlap_dist |>
  group_by(Class) |>   # Replace with your actual column name (e.g., "land_type")
  summarise(total_area_km2 = sum(area_sqkm, na.rm = TRUE))

intersection_final <- st_transform(intersection_overlap_dist, crs_epsg)

print(summary)

sum <- sum(summary$total_area_km2)
...
```{r}
plot(
  intersection_final["Class"],
  lwd = 0.2,                    # thinner lines
  border = "white",            # light borders help define shapes
  pal = viridis::viridis,        # optional: change color palette
  main = "Species Overlap by Ownership Class"  # add a title
)

ggplot() +
  geom_raster(data = env_df, aes(x = x, y = y, fill = env)) +
  scale_fill_gradient(name = "Temperature", low = "black", high = "white") +
  ggnewscale::new_scale_fill() +
  geom_sf(data = intersection_final, aes(fill = Class, color = Class), 
          size = 0.01, alpha = 0.7) +
  scale_fill_viridis_d(name = "Land Ownership") +
  scale_color_viridis_d(guide = "none")+
  theme_minimal() +
  labs(title = "High Sampling Priority Regions for Diplaucus areugsjfd",
       subtitle = "Where variablity is in the top 95% and species occurence is expected",
       x = "",
       y="")

#subtitle = "40.5% Of Regions on Land Owned by Home Owners Associations, Non-Profits, or other Private entities, 59.5% Of Regions on Land owned by City, County, Federal, State, and Tribal governments")
```
