...
```{r}
replicate_run <- function(n_bootstrap, species_name, predictors, modelmethod, varimp_list, predictions_list, final_model, clean_occ_sf, bg_df, i) { #do this for however many times you want to bootstrap
  ...
  predictors <- unwrap(predictors)
    ...
    i = i+1
    message("  Bootstrap ", i, "/", n_bootstrap)
  ...
  point_count <- nrow(as.data.frame(sdm_data_pts))
    ...
    predictions_list[[i]] <- predict(model, data = predictors, type = "cloglog") #creates a prediction for each model and stores it
...
#read terra raster into memory so cpus arent all pulling from memory version
  ...
  replicate_run(n_bootstrap, species_name, predictors, modelmethod, varimp_list, predictions_list, final_model, clean_occ_sf, bg_df, i)
  
  #availableCores()
  plan(multisession, workers = 20)

  future_map(.x = 1:n_bootstrap, .f = replicate_run, n_bootstrap=n_bootstrap, species_name=species_name, predictors=predictors, modelmethod=modelmethod, varimp_list=varimp_list, predictions_list=predictions_list, final_model=final_model, clean_occ_sf=clean_occ_sf, bg_df=bg_df, i=i, .progress = TRUE, .options = furrr_options(seed = TRUE))
