...
```{r}
results_dir <- "~/R/Results/Quercus/07Jul25" # <-- CHANGE this to your desired output directory
data_dir <- "~/R/Data/Quercus/07Jul25" # <-- CHANGE, this is where you will put all the data you used for this SDM
...
### Replicate Function to nest inside full bootstrap
```{r}
...
#read terra raster into memory so cpus arent all pulling from memory version
  #multithreading/parallelization terra raster (wrap?)
  ...
  # replicate_run(n_bootstrap, species_name, predictors, modelmethod, clean_occ_sf, bg_df)
  ...
  # map(.x = 1:2,  replicate_run(x, n_bootstrap=n_bootstrap, species_name=species_name, predictors=predictors, modelmethod=modelmethod,clean_occ_sf=clean_occ_sf, bg_df=bg_df), .progress = TRUE, .options = furrr_options(seed = TRUE))
  # future_map(.x = 1:n_bootstrap, .f = replicate_run, n_bootstrap=n_bootstrap, species_name=species_name, predictors=predictors, modelmethod=modelmethod,clean_occ_sf=clean_occ_sf, bg_df=bg_df, .progress = TRUE, .options = furrr_options(seed = TRUE))
  toc()
